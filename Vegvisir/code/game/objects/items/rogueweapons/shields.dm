...
/obj/item/rogueweapon/shield
	...
	wdefense = 8										//Was 15, lowered to 5 - Reason: Shield skill now handles your better parrying w/ shields.
	var/coverage = 90
...
/obj/item/rogueweapon/shield/wood/attack_hand(mob/user)
	if(!overlays.len)
		...
		var/picked_name = input(user, "Choose a Heraldry", "Vegvisir Keep", name) as null|anything in sortList(istates)
		if(!picked_name)
			picked_name = "none"
...
/obj/item/rogueweapon/shield/buckler
	...
	dropshrink = 0.8
	resistance_flags = null
	wdefense = 8		//Covers less than tower, so more arrows go through, but are more made for parrying.
	coverage = 10
	attacked_sound = list('sound/combat/parry/shield/metalshield (1).ogg','sound/combat/parry/shield/metalshield (2).ogg','sound/combat/parry/shield/metalshield (3).ogg')
	parrysound = list('sound/combat/parry/shield/metalshield (1).ogg','sound/combat/parry/shield/metalshield (2).ogg','sound/combat/parry/shield/metalshield (3).ogg')
	max_integrity = 300
