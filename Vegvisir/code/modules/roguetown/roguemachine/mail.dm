...
/obj/structure/roguemachine/mail/attack_right(mob/user)
	...
	if(!coin_loaded)
		...
		return
	var/send2place = input(user, "Where to? (Person or #number)", "Vegvisir Keep", null)
	if(!send2place)
		return
	var/sentfrom = input(user, "Who is this letter from?", "Vegvisir Keep", null)
	if(!sentfrom)
		sentfrom = "Anonymous"
	var/t = stripped_multiline_input("Write Your Letter", "Vegvisir Keep", no_trim=TRUE)
	if(t)
		if(length(t) > 2000)
...
/obj/structure/roguemachine/mail/attackby(obj/item/P, mob/user, params)
	...
	if(istype(P, /obj/item/paper) || istype(P, /obj/item/smallDelivery))
		...
		if(alert(user, "Send Mail?",,"YES","NO") == "YES")
			var/send2place = input(user, "Where to? (Person or #number)", "Vegvisir Keep", null)
			var/sentfrom = input(user, "Who is this from?", "Vegvisir Keep", null)
			if(!sentfrom)
				sentfrom = "Anonymous"
			if(findtext(send2place, "#"))
				var/box2find = text2num(copytext(send2place, findtext(send2place, "#")+1))
