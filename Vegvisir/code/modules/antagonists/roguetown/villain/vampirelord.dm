...
/obj/structure/vampire/bloodpool/attack_hand(mob/living/user)
	...
	if(user.mind.special_role != "Vampire Lord")
		return
	var/choice = input(user,"What to do?", "Vegvisir Keep") as anything in useoptions|null
	switch(choice)
		if("Grow Power")
			if(lord.vamplevel == 4)
		...
		if("Shape Amulet")
			if(alert(user, "Craft a new amulet Cost:500","","Yes","No") == "Yes")
				...
				if(do_after(user, 100))
					...
					var/naming = input(user, "Select a name for the amulet.", "Vegvisir Keep") as text|null
...
/mob/dead/observer/rogue/arcaneeye/proc/scry_tele()
	...
	for(var/V in GLOB.sortedAreas)
		...
		if(!A.hidden)
			filtered += A
	var/area/thearea  = input("Area to jump to", "Vegvisir Keep") as null|anything in filtered
