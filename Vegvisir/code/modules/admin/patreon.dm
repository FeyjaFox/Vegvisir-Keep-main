...
/proc/load_patreons()
	...
	for(var/line in csvlist)
		if(findtext(line, "Vegvisir Keep SILVER"))
...
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext(line, "Vegvisir Keep GOLD"))
			if(findtext(line, "Active patron"))
				var/index = findtext(line, ",")
				var/indexs = findtext(line, ",",index+1)
				var/player_email = copytext(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext(line, "Vegvisir Keep"))
			if(findtext(line, "Active patron"))
				var/index = findtext(line, ",")
				var/indexs = findtext(line, ",",index+1)
				var/player_email = copytext(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext(line, "Vegvisir Keep MERCHANT"))
			if(findtext(line, "Active patron"))
				var/index = findtext(line, ",")
				var/indexs = findtext(line, ",",index+1)
				var/player_email = copytext(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext(line, "Vegvisir Keep LORD"))
			if(findtext(line, "Active patron"))
				var/index = findtext(line, ",")
				var/indexs = findtext(line, ",",index+1)
				var/player_email = copytext(line,index+1,indexs)
	...
	for(var/line in csvlist)
		if(findtext(line, the_email))
			if(findtext(line, "Active patron"))
				...
				if(findtext(line, "Vegvisir Keep GOLD"))
					return 2
				if(findtext(line, "Vegvisir Keep MYTHRIL"))
					return 3
				if(findtext(line, "Vegvisir Keep MERCHANT"))
					return 4
				if(findtext(line, "Vegvisir Keep LORD"))
...
//		return
	if(client.prefs.anonymize == TRUE)
		if(alert(src, "Disable Anonymize? (Not Recommended)", "Vegvisir Keep", "YES", "NO") == "YES")
		...
		maintaining privacy from other BYOND users.", "Vegvisir Keep", "YES", "NO") == "YES")
			if(GLOB.respawncounts[client.ckey])
				to_chat(src, span_warning("You have already spawned."))
				return
...
/mob/dead/new_player/proc/register_patreon()
	...
	if(client)
		if(client.patreonlevel())
			return
	var/name = input("Enter your patreon DISPLAY NAME exactly as it appears on Patreon.","Vegvisir Keep") as text|null
	if(!name)
		return
	var/email = input("Enter your patreon EMAIL ADDRESS exactly as it appears on Patreon.","Vegvisir Keep") as text|null
	if(!email)
		return
