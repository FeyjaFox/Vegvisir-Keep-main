...
"MAJENTA"="#962e5c")
	var/prim
	var/sec
	var/choice = input(src, "Choose a Primary Color", "Vegvisir Keep") as anything in lordcolors
	if(choice)
		prim = lordcolors[choice]
		lordcolors -= choice
	choice = input(src, "Choose a Secondary Color", "Vegvisir Keep") as anything in lordcolors
	if(choice)
